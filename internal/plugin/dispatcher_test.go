...
func TestRegistrationAdvertisesOnlyRequiredCapabilities(t *testing.T) {
	...
	if raw["schema_version"] != float64(pluginabi.SchemaVersion) {
		t.Fatalf("schema_version = %#v, want %d", raw["schema_version"], pluginabi.SchemaVersion)
	}
...
func TestCPAContractVersions(t *testing.T) {
	...
	if pluginabi.SchemaVersion != 2 {
		t.Fatalf("schema version = %d, want 2", pluginabi.SchemaVersion)
	}
}

func TestLifecycleMethodsAcceptSchemaVersionTwo(t *testing.T) {
	...
	for _, method := range []string{pluginabi.MethodPluginRegister, pluginabi.MethodPluginReconfigure} {
		t.Run(method, func(t *testing.T) {
			envelope := decodeTestEnvelope(t, dispatcher.Dispatch(method, []byte(`{"schema_version":2}`)))
			...
			if registration.SchemaVersion != 2 {
				t.Fatalf("response schema version = %d, want 2", registration.SchemaVersion)
			}
		})
	}
}
...
func TestLifecycleMethodsRejectInvalidOrUnsupportedSchemas(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "schema v1", request: `{"schema_version":1}`, code: "unsupported_schema", message: "plugin schema version 1 is not supported"},
		{name: "missing schema", request: `{}`, code: "unsupported_schema", message: "plugin schema version 0 is not supported"},
		{name: "zero schema", request: `{"schema_version":0}`, code: "unsupported_schema", message: "plugin schema version 0 is not supported"},
		{name: "empty request", request: ``, code: "unsupported_schema", message: "plugin schema version 0 is not supported"},
		{name: "malformed JSON", request: `{"schema_version":`, code: "invalid_request", message: "plugin lifecycle request is invalid"},
		{name: "future schema", request: `{"schema_version":3}`, code: "unsupported_schema", message: "plugin schema version 3 is not supported"},
	}
	for _, method := range []string{pluginabi.MethodPluginRegister, pluginabi.MethodPluginReconfigure} {
		method := method
